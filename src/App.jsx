...
const firebaseConfig = {
  apiKey: "AIzaSyAs-your-actual-key-here", // Placeholder if not provided, but the IDs below match your project
...
// Mapping for recovery (Old Name -> New Name)
const RECOVERY_MAP = {
...
export default function App() {
  ...
  const [statusLogs, setStatusLogs] = useState(["System Ready. Enter credentials."]);
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        // Fallback to anonymous if custom token isn't present
  ...
  useEffect(() => {
    ...
    // Recovery Path (Old collection structure)
    ...
    const unsubscribe = onSnapshot(docRef, async (snap) => {
      if (snap.exists()) {
        ...
        addLog(`Loaded: ${selectedSheetDate} (${viewMode})`);
        setIsSyncing(false);
      } else {
        // Try Recovery
        addLog(`Searching legacy database for ${selectedSheetDate}...`);
        try {
            ...
            if (legacySnap.exists()) {
              ...
              addLog("No records found. New ledger.");
            }
        } catch (e) {
            addLog("Database Scan Error");
        }
        setIsSyncing(false);
      }
    }, (err) => {
      setIsSyncing(false);
      addLog(`Database Error: ${err.code}`);
    });
      ...
      <div className="w-full max-w-md bg-white rounded-[3rem] p-12 shadow-2xl">
        ...
        <form onSubmit={(e) => {
          ...
          if (d.get('id').toLowerCase() === ADMIN_CREDENTIALS.loginId && d.get('pw') === ADMIN_CREDENTIALS.password) {
            ...
            addLog("Invalid Access Credentials");
          }
        }} className="space-y-4">
          ...
          <input name="pw" type="password" placeholder="Key" required className="w-full px-6 py-4 bg-slate-50 rounded-2xl border border-slate-100 font-bold outline-none focus:ring-2 ring-indigo-500/20" />
          <button type="submit" className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-indigo-600 transition-all text-xs">Unlock Database</button>
        </form>
        <p className="mt-8 text-center text-[9px] font-black text-slate-300 uppercase tracking-widest leading-relaxed">
            All access attempts are logged.<br/>Encrypted via AES-256
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-32">
      {/* Top Header / Recovery Log */}
      <div className="bg-slate-900 text-white px-6 py-3 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
            <ShieldCheck size={16} className="text-emerald-400"/>
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 border-r border-slate-700 pr-3 mr-1">Status:</span>
            <span className="text-[10px] font-black uppercase text-emerald-400 animate-pulse">{statusLogs[0]}</span>
        </div>
        <button onClick={() => setIsAuthenticated(false)} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white transition-colors flex items-center gap-2">
      ...
      <header className="max-w-6xl mx-auto mt-12 px-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex items-center justify-between">
          ...
          }} className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all border border-slate-100"><ChevronLeft size={28}/></button>
          ...
          <button onClick={() => {
            ...
            date.setDate(date.getDate() + 7);
            setSelectedSheetDate(formatDate(date));
          }} className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all border border-slate-100"><ChevronRight size={28}/></button>
        </div>

        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col justify-center">
             <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] mb-2">Net Cash Flow</p>
      ...
      <main className="max-w-6xl mx-auto mt-10 px-6 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <section>
          ...
          <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm space-y-5">
            <div className="p-5 bg-slate-50 rounded-2xl flex items-center justify-between border border-slate-100 mb-4">
                 <div className="flex items-center gap-3">
                    <History size={16} className="text-slate-400" />
                    <span className="text-[10px] font-black uppercase text-slate-400">Previous Balance</span>
                 </div>
            ...
            {REVENUE_CATEGORIES.map(cat => (
              <div key={cat} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                <div className="flex flex-col">
                    ...
                    <span className="text-[9px] font-bold text-slate-400 uppercase">{viewMode} Entry</span>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <input type="number" value={revenueData[cat] || ''} disabled={isSubmitted}
                        ...
                        }} className="w-32 py-2 font-black text-right outline-none text-lg text-slate-900 placeholder-slate-200" placeholder="0.00" />
                </div>
              </div>
            ))}
          </div>
              ...
              {!isSubmitted && (
                <form onSubmit={(e) => {
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase px-1">Select Account</label>
                    <select name="c" className="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs font-black uppercase outline-none">
                        {EXPENSE_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                  </div>
                  <div className="flex gap-2">
                      <input name="a" type="number" step="0.01" required placeholder="Amount 0.00" className="flex-1 bg-white border border-slate-200 rounded-xl p-3 text-sm font-black outline-none focus:ring-2 ring-indigo-500/20" />
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {expenses.length === 0 ? (
                  <div className="py-20 flex flex-col items-center justify-center text-slate-300 gap-3">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">No entries for this period</span>
                  </div>
                ) : (
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-full max-w-md px-6">
        <button 
          onClick={() => { if(window.confirm("Lock this report for the permanent record?")) handleSave({ status: 'submitted' }); }}
          disabled={isSubmitted || isSyncing}
          className={`w-full py-6 rounded-3xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-4 shadow-2xl transition-all ${isSubmitted ? 'bg-emerald-500 text-white shadow-emerald-200' : 'bg-slate-900 text-white hover:bg-indigo-600 active:scale-95'}`}
        >
          {isSubmitted ? <CheckCircle2 size={20}/> : <BarChart3 size={20}/>}
          {isSubmitted ? 'Report Finalized' : 'Finalize & Post Record'}
        </button>
      </div>
