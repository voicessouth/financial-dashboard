...
export default function App() {
  ...
  // --- 2. DATA LISTENER & RESET (FIX FOR DATA OVERLAP) ---
  useEffect(() => {
    if (!user || !isAuthenticated) return;

    // STEP A: RESET UI IMMEDIATELY ON CHANGE
    // This prevents Sunday data from staying in view while Tue is loading
    setIsSyncing(true);
    setRevenueData({});
    setExpenses([]);
    ...
    // PATH: /artifacts/{appId}/public/data/data/{docId}
    ...
    const unsubscribe = onSnapshot(docRef, 
      (snap) => {
        if (snap.exists()) {
          ...
          addLog(`SYNC: Data loaded for ${viewMode}`);
        } else {
          addLog(`NEW: Creating record for ${viewMode}`);
        }
        setIsSyncing(false);
      }, 
      (err) => {
    ...
    <div className="min-h-screen bg-[#f8fafc] pb-32 font-sans">
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex gap-4 items-center">
             <div className="flex flex-col items-end mr-4">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Connection Status</span>
                <span className={`flex items-center gap-1.5 text-[10px] font-black uppercase ${isSyncing ? 'text-amber-500' : 'text-emerald-500'}`}>
                  ...
                  {isSyncing ? 'Synchronizing' : 'Cloud Sync Active'}
                </span>
             </div>
      ...
      {permissionError && (
        <div className="max-w-6xl mx-auto mt-6 px-6">
          <div className="bg-rose-600 border border-rose-700 p-5 rounded-[2rem] flex items-center justify-between shadow-xl shadow-rose-200 animate-bounce">
            <div className="flex items-center gap-4 text-white">
              <AlertCircle size={24}/>
              <div>
                <p className="text-xs font-black uppercase tracking-wider">Access Denied</p>
                <p className="text-[10px] font-bold uppercase opacity-80">Check Firebase Permission Rules</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <header className="max-w-6xl mx-auto mt-8 px-6 flex flex-col lg:flex-row gap-6">
        <div className="flex-1 bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm flex items-center justify-between">
          ...
          <div className="text-center">
            <p className="text-[11px] font-black text-indigo-500 uppercase tracking-[0.2em] mb-1">{viewMode} Entries</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="bg-white px-8 py-6 rounded-[2.5rem] border border-slate-200 text-center flex-1">
             <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">Income</p>
             <p className="text-xl font-black text-emerald-600">${totals.rev.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
          </div>
          <div className="bg-white px-8 py-6 rounded-[2.5rem] border border-slate-200 text-center flex-1">
             <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">Expenses</p>
             <p className="text-xl font-black text-rose-600">${totals.exp.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
          </div>
          <div className="bg-slate-900 px-10 py-6 rounded-[2.5rem] text-center text-white flex-1 shadow-xl">
             <p className="text-[9px] font-black uppercase text-indigo-300 tracking-widest mb-1">Final Total</p>
      ...
      <main className="max-w-6xl mx-auto mt-8 px-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden">
          {isSyncing && (
            <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-10 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Loading Records...</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <h3 className="font-black text-sm uppercase tracking-widest text-slate-800">Income Log</h3>
          </div>
          
          <div className="mb-8 p-6 bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-between">
            <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Carry Over Balance</span>
              ...
              <div key={cat} className="flex items-center gap-4 group">
                ...
                <div className="relative w-40">
                  ...
                  <input type="number" value={revenueData[cat] || ''} disabled={isSubmitted}
                    ...
                    }} className="w-full pl-8 pr-4 py-3 bg-slate-50 rounded-2xl font-black text-right text-slate-700 outline-none border border-transparent focus:border-indigo-500 focus:bg-white transition-all shadow-sm group-hover:shadow-md" placeholder="0.00" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 space-y-3 mb-8 overflow-y-auto max-h-[450px] pr-2 custom-scrollbar">
            {expenses.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center opacity-30 py-24 text-center">
                ...
                <p className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">Empty Ledger</p>
              </div>
            ) : (
                ...
                <div key={idx} className="bg-slate-50 p-5 rounded-2xl flex justify-between items-center group border border-slate-100 hover:border-rose-100 hover:bg-white transition-all">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {!isSubmitted && (
                      ...
                      }} className="text-slate-200 hover:text-rose-500 transition-all opacity-0 group-hover:opacity-100"><XCircle size={16}/></button>
                    )}
                  </div>
                </div>
              ))
          ...
          {!isSubmitted && (
            <form onSubmit={(e) => {
              ...
              <div className="flex gap-4">
                <select name="c" className="flex-1 py-4 px-5 bg-slate-800 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest outline-none ring-indigo-500 focus:ring-2 appearance-none">
                ...
                <div className="relative w-32">
                  ...
                  <input name="a" type="number" step="0.01" required placeholder="0.00" className="w-full py-4 pl-8 pr-4 bg-slate-800 text-white rounded-2xl text-xs font-black outline-none ring-indigo-500 focus:ring-2" />
                </div>
      ...
      <div className="max-w-6xl mx-auto mt-12 px-6">
        <div className="flex items-center gap-3 mb-4">
           ...
           <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.25em]">Cloud Connectivity Stream</h4>
        </div>
        <div className="bg-white/50 backdrop-blur rounded-[2rem] p-6 border border-slate-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {logs.map((log, i) => (
            <div key={i} className={`flex items-center gap-3 py-1 text-[9px] font-black uppercase ${log.includes('ERR') || log.includes('CRITICAL') ? 'text-rose-500' : 'text-slate-400'}`}>
      ...
      <footer className="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-sm px-6 z-50">
        ...
        >
          ...
          {isSubmitted ? 'Ledger Sealed' : 'Verify & Lock Report'}
        </button>
      </footer>
